...
class NotificationService {
  ...
  static Future<void> _createAndroidChannels() async {
    ...
    final soundEnabled = prefs.getBool('diyaa-sound-enabled') ?? true;

    // Delete channels before recreating them.
  ...
  /// Request the exact alarm permission explicitly.
  /// Can be triggered by the user via settings if they want to ensure absolute
  /// precision, without forcing an automatic redirect on startup/on-resume.
  static Future<bool> requestExactAlarmsPermissionExplicitly() async {
  ...
  static Future<void> scheduleAll(
    ...
    // Recreate Android channels dynamically to ensure that channel configurations (like playSound)
    // always accurately match the user's latest sound setting.
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // TEST AZKAR NOTIFICATION
  // ══════════════════════════════════════════════════════════════════════════
  static Future<void> sendTestAzkarNotification({
    bool isArabic = false,
    ...
    try {
      ...
      debugPrint('[Notifications] TEST AZKAR FAILED: $e');
    }
  }
  ...
  // ══════════════════════════════════════════════════════════════════════════
  static Future<void> sendTestStreakNotification({
    bool isArabic = false,
    int currentStreak = 1,
    bool soundEnabled = true,
  }) async {
    ...
    try {
      ...
      await _plugin.show(
        id: 96,
        title: isArabic
            ? 'لا تكسر سلسلتك ($streak يوم)'
            : "Don't break your streak! ($streak days)",
        body: isArabic
      ...
      debugPrint('[Notifications] TEST STREAK FAILED: $e');
    }
  }
