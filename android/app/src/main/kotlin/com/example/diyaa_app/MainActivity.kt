...
class MainActivity : FlutterActivity() {
    ...
    private val ICON_CHANNEL_NAME = "diyaa_icon_channel"
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, ICON_CHANNEL_NAME)
            .setMethodCallHandler { call, result ->
                if (call.method == "changeIcon") {
                    val darkMode = call.argument<Boolean>("darkMode") ?: return@setMethodCallHandler result.error(
                        "INVALID_ARGS", "Missing 'darkMode' argument", null
                    )
                    changeIcon(darkMode, result)
                } else {
    ...
    /**
     * Switch active launcher components dynamically between MainActivityLight and MainActivityDark.
     * Uses packageManager.setComponentEnabledSetting to enable one and disable the other.
     * Handles exceptions and ensures the app does not crash.
     */
    private fun changeIcon(darkMode: Boolean, result: MethodChannel.Result) {
        try {
            val packageManager = packageManager
            val lightComponent = ComponentName(this, "com.example.diyaa_app.MainActivityLight")
            val darkComponent = ComponentName(this, "com.example.diyaa_app.MainActivityDark")

            val enableComponent = if (darkMode) darkComponent else lightComponent
            val disableComponent = if (darkMode) lightComponent else darkComponent

            val currentSetting = packageManager.getComponentEnabledSetting(enableComponent)
            if (currentSetting == PackageManager.COMPONENT_ENABLED_STATE_ENABLED) {
                Log.d(TAG, "Icon component ${if (darkMode) "Dark" else "Light"} is already enabled. Skipping.")
                result.success(true)
                return
            }
            ...
            Log.d(TAG, "Successfully changed active icon component to ${if (darkMode) "Dark" else "Light"}")
            result.success(true)
        } catch (e: Exception) {
            Log.e(TAG, "Failed to change icon component: ${e.message}", e)
